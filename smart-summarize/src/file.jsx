...
// In production: same origin (reverse proxy) or set VITE_API_URL env var
...
const GlobalStyle = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .hero-sub { font-size: 1.1rem; color: var(--text2); max-width: 560px; margin: 0 auto 2.5rem; line-height: 1.7; }
    ...
    .section-title { font-family: 'Syne', sans-serif; font-size: clamp(1.5rem, 3vw, 2rem); font-weight: 700; }
    .section-sub { color: var(--text2); margin-top: 0.75rem; font-size: 0.9rem; }
    ...
    .feature-title { font-family: 'Syne', sans-serif; font-size: 0.95rem; font-weight: 600; margin-bottom: 0.5rem; }
    ...
    .waveform { height: 50px; display: flex; align-items: center; gap: 2px; overflow: hidden; margin: 1rem 0; }
    .wave-bar {
      flex: 1; background: linear-gradient(to top, var(--accent), #7c3aed);
      border-radius: 2px; min-height: 3px; transform-origin: bottom; transition: height 0.1s;
    }
    .wave-bar.playing { animation: wave var(--dur, 0.8s) ease-in-out infinite alternate; }
    ...
    .play-btn {
      ...
      font-size: 1.1rem; color: #fff; transition: all 0.2s; flex-shrink: 0;
      box-shadow: 0 0 20px rgba(79,125,255,0.4);
    }
    .play-btn:hover { transform: scale(1.08); box-shadow: 0 0 30px rgba(79,125,255,0.6); }
    .audio-progress { flex: 1; }
    .progress-bar { width: 100%; height: 4px; background: var(--surface2); border-radius: 2px; overflow: hidden; cursor: pointer; }
    .progress-fill { height: 100%; background: linear-gradient(90deg, var(--accent), #7c3aed); border-radius: 2px; transition: width 0.1s; }
    .time-row { display: flex; justify-content: space-between; font-size: 0.75rem; color: var(--text3); margin-top: 0.35rem; }
    .voice-select {
      background: var(--surface); border: 1px solid var(--border); border-radius: 8px;
      color: var(--text2); padding: 0.4rem 0.75rem; font-size: 0.82rem;
    ...
    .price-features li { font-size: 0.84rem; color: var(--text2); display: flex; align-items: center; gap: 0.5rem; }
    ...
    .faq-q { padding: 1.1rem 1.25rem; cursor: pointer; display: flex; justify-content: space-between; align-items: center; font-size: 0.9rem; font-weight: 500; user-select: none; transition: background 0.2s; }
    ...
    <>
      <div className="hero reveal" style={{ maxWidth: 960, margin: "0 auto", padding: "6rem 2rem 4rem" }}>
        {user ? (
          ...
          <div className="hero-badge">
            ...
            Read Less. Understand More. Remember Everything.
          </div>
        )}
        <h1>Your Books, Distilled<br />Into Pure Intelligence</h1>
        <p className="hero-sub">Upload any book, research paper, or document. BookMind extracts the essence — delivering deep summaries, key insights, and audio narration in seconds.</p>
        ...
        <div className="stats-row" style={{ marginTop: "3rem", maxWidth: 560, marginLeft: "auto", marginRight: "auto" }}>
          <div className="stat-card"><div className="stat-num"><Counter target={24800} suffix="+" /></div><div className="stat-label">Docs Processed</div></div>
          <div className="stat-card"><div className="stat-num"><Counter target={96} suffix="%" /></div><div className="stat-label">Accuracy Rate</div></div>
          <div className="stat-card"><div className="stat-num"><Counter target={18} suffix="x" /></div><div className="stat-label">Time Saved</div></div>
        </div>
      </div>
      ...
      <section style={{ maxWidth: 960, margin: "0 auto", padding: "4rem 2rem" }}>
        <div className="section-header reveal">
          ...
          <p className="section-sub">Enterprise-grade AI summarization with an intuitive interface</p>
        </div>
        <div className="features-grid">
          {[
            ["🧠","Deep Comprehension Engine","Industry-leading language understanding with deep context preservation, nuance detection, and accuracy across any domain or document type."],
            ["⚡","Smart Chunking","Process documents up to 1500 pages with intelligent chunking that preserves context and merges summaries coherently."],
            ["🎙","Audio Narration","Convert summaries to lifelike audio in multiple voice styles with speed control, waveform visualization, and MP3 export."],
            ["📊","Keyword Extraction","Automatically surface key concepts, entities, and themes from your documents for quick scanning and reference."],
            ["🔒","Private & Secure","Your documents are encrypted in transit, processed ephemerally, and never stored or used for training."],
            ["📤","Flexible Export","Download summaries as PDF, DOCX, or TXT. Export audio as high-quality MP3. Share instantly via link."],
          ].map(([icon, title, desc]) => (
      ...
      <section style={{ maxWidth: 960, margin: "0 auto", padding: "4rem 2rem" }}>
        <div className="section-header reveal">
          ...
          <h2 className="section-title">Loved by Researchers & Students</h2>
        </div>
        <div className="testimonial-grid">
          {[
            ["AR","Dr. Ananya Roy","AI Research Lead, IIT Bombay","BookMind cut my research time by 80%. I upload 300-page papers and get perfectly structured summaries in under a minute. It's become essential to my workflow."],
            ["MK","Mihir Kulkarni","Corporate Lawyer, Mumbai","The audio summaries are a game changer. I listen to complex legal briefs during my commute. The voice quality is indistinguishable from a professional narrator."],
            ["PS","Priya Sharma","MBA Student, IIM Ahmedabad","Our entire MBA cohort uses BookMind for case studies. The chapter-wise summaries and keyword extraction are exactly what academic presentations demand."],
            ["VN","Vijay Narayanan","Senior Engineer, TCS","Processing technical manuals used to take days. Now I get accurate, detailed summaries in minutes with the right technical keywords highlighted."],
      ...
      <section style={{ maxWidth: 960, margin: "0 auto", padding: "4rem 2rem" }}>
        ...
        <div className="faq-list reveal">
          ...
          <FaqItem q="What audio voices and languages are available?" a="We currently offer 4 English voice styles (Alloy, Nova, Echo, Shimmer). Multi-language support including Hindi, Spanish, French, and German is coming in Q3 2026." />
          <FaqItem q="What file formats are supported?" a="Currently PDF, DOC/DOCX, and TXT files up to 50MB. Support for EPUB, HTML, Markdown, and PowerPoint is on our Q3 2026 roadmap." />
        </div>
      </section>
...
function UploadPage() {
  ...
  const startProgressAnimation = () => {
    // Slowly inch progress from 5% to 90% while backend works
    // Real completion jumps to 100% when response arrives
    let current = 5;
    const steps = [
      ...
      { target: 90, label: "Extracting key insights and keywords...", delay: 2000 },
    ];
  ...
  const handleGenerate = async () => {
    ...
    console.log("[Frontend] handleGenerate start, file:", file?.name, "mode:", mode, "length:", lengthLabels[length - 1]);
    ...
    try {
      ...
      if (file) {
        ...
        console.log("[Frontend] File appended to FormData:", file.name, file.size, "bytes");
      }
      formData.append("mode", mode);
      formData.append("lengthLabel", lengthLabels[length - 1]);

      console.log("[Frontend] Sending POST to", `${API_BASE}/api/summarize`);
      ...
      console.log("[Frontend] Response parsed, success:", data.success);
      ...
      // ── Navigate to result ────────────────────────────────────────────
      console.log("[Frontend] Navigating to result page");
      navigate("/result", {
        state: {
      ...
      if (err.name === "AbortError") {
        ...
        setErrorMsg("Cannot reach the backend server. Make sure the server is running on port 3001.");
      } else {
    ...
    <div style={{ maxWidth: 960, margin: "0 auto", padding: "3rem 2rem 5rem" }}>
      ...
      >
        ...
        <div className="file-types">
          ...
          <span className="file-badge">Up to 50MB</span>
        </div>
      </div>
      ...
      <div className="controls-grid reveal">
        <div className="control-group">
          <div className="control-label">Summary Type</div>
          <div className="mode-btns">
            {["concise", "detailed", "bullets"].map((m) => (
              <button key={m} className={`mode-btn${mode === m ? " active" : ""}`} onClick={() => setMode(m)} disabled={loading}>
      ...
      {/* Loading state — stays visible until backend responds */}
      {loading && (
        <>
      ...
      {/* Error — shown only after loading ends with failure */}
      {!loading && errorMsg && (
    ...
    <div className="waveform">
      {barsRef.current.map((b, i) => (
        <div key={i} className={`wave-bar${playing ? " playing" : ""}`}
          style={{ height: `${b.height}%`, "--dur": `${b.dur}s`, animationDelay: `${b.delay}s` }} />
      ))}
    </div>
  );
}
...
function SummaryResultPage() {
  ...
  const [voice, setVoice] = useState("Alloy (Default)");
  ...
  useEffect(() => {
    const load = () => {
      const v = getSynth().getVoices().filter((v) => v.lang.startsWith("en"));
      if (v.length > 0) setVoices(v);
    };
    load();
    getSynth().onvoiceschanged = load;
    return () => { getSynth().onvoiceschanged = null; };
  }, []);

  const voiceOptions = [
    { label: "Alloy (Default)", idx: 0, pitch: 1.0 },
    { label: "Nova (Warm)",     idx: 1, pitch: 1.1 },
    { label: "Echo (Deep)",     idx: 2, pitch: 0.8 },
    { label: "Shimmer (Clear)", idx: 3, pitch: 1.2 },
  ];

  const wordCount = summary.split(/\s+/).length;
  useEffect(() => {
    setAudioDuration(Math.round((wordCount / (speed * 150)) * 60));
  }, [wordCount, speed]);
  ...
  const startProgressTick = (speedVal) => {
    clearInterval(progressIntervalRef.current);
    startTimeRef.current = Date.now();
    progressIntervalRef.current = setInterval(() => {
      const elapsed = elapsedRef.current + (Date.now() - startTimeRef.current) / 1000;
      const dur = Math.round((wordCount / (speedVal * 150)) * 60);
      if (elapsed >= dur) {
        clearInterval(progressIntervalRef.current);
        setAudioProgress(dur);
        setAudioPlaying(false);
        isSpeakingRef.current = false;
  ...
  const getSelectedVoice = (voiceLabel, voiceList) => {
    const opt = voiceOptions.find((v) => v.label === voiceLabel) ?? voiceOptions[0];
    if (voiceList.length === 0) return null;
    return voiceList[opt.idx % voiceList.length] ?? voiceList[0];
  };

  const getSelectedPitch = (voiceLabel) =>
    (voiceOptions.find((v) => v.label === voiceLabel) ?? voiceOptions[0]).pitch;

  const speakFrom = (sentenceIdx, speedVal, voiceLabel) => {
    getSynth().cancel();
    ...
    const speakNext = (idx) => {
      ...
      const selectedVoice = getSelectedVoice(voiceLabel, voices);
      if (selectedVoice) utter.voice = selectedVoice;
      utter.rate = speedVal;
      utter.pitch = getSelectedPitch(voiceLabel);
      utter.volume = 1.0;
      utter.onend = () => { sentenceIdxRef.current = idx + 1; speakNext(idx + 1); };
      utter.onerror = (e) => {
        if (e.error === "interrupted" || e.error === "canceled") return;
        setAudioError("Speech error: " + e.error);
        isSpeakingRef.current = false;
        setAudioPlaying(false);
        clearInterval(progressIntervalRef.current);
      };
      utteranceRef.current = utter;
      getSynth().speak(utter);
    };
    sentenceIdxRef.current = sentenceIdx;
    setAudioPlaying(true);
    startProgressTick(speedVal);
    speakNext(sentenceIdx);
  };
  ...
  const togglePlay = () => {
    ...
    if (audioPlaying) {
      getSynth().pause();
      clearInterval(progressIntervalRef.current);
      elapsedRef.current = audioProgress;
      isSpeakingRef.current = false;
      setAudioPlaying(false);
    } else {
      if (sentencesRef.current.length === 0) {
        sentencesRef.current = summary.match(/[^.!?\n]+[.!?\n]+/g) || [summary];
      }
      if (getSynth().paused) {
        getSynth().resume();
        startTimeRef.current = Date.now();
        startProgressTick(speed);
        isSpeakingRef.current = true;
        setAudioPlaying(true);
      } else {
        speakFrom(sentenceIdxRef.current, speed, voice);
      }
    }
  };
  ...
  const handleVoiceChange = (e) => {
    ...
    if (isSpeakingRef.current) {
      ...
      stopAll();
      setTimeout(() => speakFrom(sentenceIdxRef.current, speed, v), 100);
    }
  };
  ...
  const handleSpeedChange = (e) => {
    ...
    setSpeed(s);
    setAudioDuration(Math.round((wordCount / (s * 150)) * 60));
    if (isSpeakingRef.current) {
      elapsedRef.current += (Date.now() - startTimeRef.current) / 1000;
      stopAll();
      setTimeout(() => speakFrom(sentenceIdxRef.current, s, voice), 100);
    }
  };
  ...
  const handleSeek = (e) => {
    ...
    const seekSentenceIdx = Math.floor(ratio * (sentencesRef.current.length || 1));
    elapsedRef.current = seekTo;
    setAudioProgress(seekTo);
    if (isSpeakingRef.current) {
      stopAll();
      setTimeout(() => speakFrom(seekSentenceIdx, speed, voice), 80);
    } else {
      sentenceIdxRef.current = seekSentenceIdx;
    }
  };

  // Use backend-provided keywords if available, else compute from summary
        ...
        <div className="reveal">
          <div className="audio-player">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontSize: "0.85rem", fontWeight: 500 }}>AI Voice Narration</div>
              <select className="voice-select" value={voice} onChange={handleVoiceChange}>
                {voiceOptions.map((v) => (
            ...
            {audioError && (
              <div style={{ fontSize: "0.78rem", color: "#ff8080", marginTop: "0.5rem", padding: "0.5rem 0.75rem", background: "rgba(255,100,100,0.08)", borderRadius: 6 }}>
                {audioError}
              </div>
            )}
            <Waveform playing={audioPlaying} />
            <div className="audio-controls">
              <button className="play-btn" onClick={togglePlay}>{audioPlaying ? "⏸" : "▶"}</button>
              <div className="audio-progress">
                <div className="progress-bar" onClick={handleSeek} style={{ cursor: "pointer" }}>
                  <div className="progress-fill" style={{ width: `${Math.min((audioProgress / Math.max(audioDuration, 1)) * 100, 100)}%` }} />
                </div>
              ...
              <select className="voice-select" style={{ padding: "0.3rem 0.5rem", fontSize: "0.78rem" }} value={speed} onChange={handleSpeedChange}>
            ...
            <button className="export-btn" style={{ marginTop: "1rem", width: "100%" }} onClick={exportTxt}>
    ...
    <div style={{ maxWidth: 960, margin: "0 auto", padding: "3rem 2rem 5rem" }}>
      ...
      <div style={{ marginTop: "4rem" }}>
        ...
        <div className="faq-list reveal">
          ...
          <FaqItem q="Is there a free trial for Pro?" a="Yes! We offer a 7-day free trial for the Pro plan. No credit card required to start." />
